...
/** Google Translate language codes supported for auto-translation. */
...
export function languageLabel(lang: LanguageOption): string {
  if (lang.nativeName === lang.name) return lang.name;
  return `${lang.name} (${lang.nativeName})`;
}

export function findLanguage(code: string | null | undefined): LanguageOption | null {
  if (!code) return null;
  return UI_LANGUAGES.find((l) => l.code === code) ?? null;
}
